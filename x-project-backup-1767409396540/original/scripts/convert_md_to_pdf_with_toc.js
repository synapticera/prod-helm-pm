...
async function generatePdfWithOutline() {
  ...
  const tempPdfFile = path.join(__dirname, '../outputdocs/Healthcare-Sales-Reference-temp.pdf');
  ...
  try {
    ...
    pdfDoc.setSubject('Process Mining, Task Mining, and Salesforce Org Analysis for Healthcare');
    pdfDoc.setKeywords(['Healthcare', 'Sales', 'Process Mining', 'Task Mining', 'Salesforce', 'Digital Transformation']);
    ...
    console.log('✓ PDF generated successfully with metadata: Healthcare-Sales-Reference.pdf');
    console.log(`  - ${pdfDoc.getPageCount()} pages`);
    console.log(`  - ${headings.length} headings indexed`);
    console.log('');
    console.log('Note: For full Table of Contents support in Mac Preview, consider using:');
    console.log('  pandoc outputdocs/Healthcare-Sales-Reference.md -o outputdocs/Healthcare-Sales-Reference.pdf --toc --pdf-engine=wkhtmltopdf');
    console.log('  (requires wkhtmltopdf installation: brew install wkhtmltopdf)');
