...
async function readExistingCustomerData() {
  try {
    ...
    if (smbRow && midRow && entRow) {
      ...
      console.log(
        "✓ Successfully read existing customer data from spreadsheet"
      );
...
async function generateRevenueModel() {
  ...
  sheet.getCell(row, 1).value =
    "With Free-to-Paid Conversion Model & Quarterly Totals";
  ...
  sheet.getCell(row, 3).value =
    "Free users who convert to paid in following month";
  sheet.getCell(row, 3).font = { italic: true, size: 9 };
  ...
  function addQuarterlySums(
    ...
    sheet.getCell(row, quarterCols.total_2026).fill = {
      type: "pattern",
      pattern: "solid",
    ...
    sheet.getCell(row, quarterCols.total_2027).fill = {
      type: "pattern",
      pattern: "solid",
      fgColor: { argb: "FFFFE699" }
    };
    ...
    sheet.getCell(row, quarterCols.grandTotal).fill = {
      type: "pattern",
      pattern: "solid",
  ...
  for (let m = 0; m < MONTHS; m++) {
    ...
    if (m === 0) {
      sheet.getCell(row, col).value = {
        formula: `${colToLetter(col)}${newSMBRow}`
      };
    } else {
      sheet.getCell(row, col).value = {
  ...
  for (let qCol of Object.values(quarterCols)) {
    const lastMonthOfPeriod =
      qCol === quarterCols.q1_2026
        ? monthStartCol + 2
        : qCol === quarterCols.q2_2026
          ? monthStartCol + 5
          : qCol === quarterCols.q3_2026
            ? monthStartCol + 8
            : qCol === quarterCols.q4_2026
              ? monthStartCol + 11
              : qCol === quarterCols.q1_2027
                ? monthStartCol + 14
                : qCol === quarterCols.q2_2027
                  ? monthStartCol + 17
                  : qCol === quarterCols.q3_2027
                    ? monthStartCol + 20
                    : qCol === quarterCols.q4_2027
                      ? monthStartCol + 23
                      : qCol === quarterCols.total_2026
                        ? monthStartCol + 11
                        : qCol === quarterCols.total_2027
                          ? monthStartCol + 23
                          : monthStartCol + 23;
    sheet.getCell(row, qCol).value = {
      formula: `${colToLetter(lastMonthOfPeriod)}${row}`
    };
  ...
  for (let m = 0; m < MONTHS; m++) {
    ...
    if (m === 0) {
      sheet.getCell(row, col).value = {
        formula: `${colToLetter(col)}${newMidRow}`
      };
    } else {
      sheet.getCell(row, col).value = {
  ...
  for (let m = 0; m < MONTHS; m++) {
    ...
    if (m === 0) {
      sheet.getCell(row, col).value = {
        formula: `${colToLetter(col)}${newEntRow}`
      };
    } else {
      sheet.getCell(row, col).value = {
  ...
  for (let m = 0; m < MONTHS; m++) {
    ...
    if (m === 0) {
      sheet.getCell(row, col).value = {
        formula: `${colToLetter(col)}${totalMRRRow}`
      };
    } else {
      sheet.getCell(row, col).value = {
