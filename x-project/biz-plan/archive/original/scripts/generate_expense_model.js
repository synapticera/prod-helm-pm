...
const infrastructureData = [
  {
    item: "Cloud Infrastructure (AWS/GCP)",
    baseAmount: 5000,
    growthRate: 0.1,
    notes: "Scales with user growth and data processing"
  },
  {
    item: "Edge LLM Development Tools",
    baseAmount: 3000,
    growthRate: 0.05,
    notes: "Model training and optimization tools"
  },
  {
    item: "Development Tools & Software",
    baseAmount: 2500,
    growthRate: 0.03,
    notes: "IDEs, collaboration tools, per-seat licenses"
  },
  {
    item: "Security & Compliance Tools",
    baseAmount: 2000,
    growthRate: 0.05,
    notes: "SOC 2, penetration testing, monitoring"
  },
  {
    item: "Data Storage & Processing",
    baseAmount: 3500,
    growthRate: 0.12,
    notes: "Database, data warehouse, analytics storage"
  },
  {
    item: "DevOps & Monitoring",
    baseAmount: 2000,
    growthRate: 0.05,
    notes: "CI/CD, APM, logging, alerting tools"
  },
  {
    item: "Office Space & Equipment",
    baseAmount: 4000,
    growthRate: 0.08,
    notes: "Rent, furniture, equipment for growing team"
  },
  {
    item: "IT & Communications",
    baseAmount: 1500,
    growthRate: 0.03,
    notes: "Internet, phones, collaboration software"
  },
  {
    item: "Insurance & Legal",
    baseAmount: 3000,
    growthRate: 0.02,
    notes: "D&O insurance, general liability, legal retainer"
  },
  {
    item: "Accounting & Finance",
    baseAmount: 2000,
    growthRate: 0.02,
    notes: "Bookkeeping, tax prep, financial systems"
  }
];

const goToMarketData = [
  {
    item: "Content Marketing & SEO",
    baseAmount: 5000,
    growthRate: 0.08,
    notes: "Blog, SEO tools, content creation"
  },
  {
    item: "Paid Advertising (Google, LinkedIn)",
    baseAmount: 8000,
    growthRate: 0.15,
    notes: "Scales with customer acquisition goals"
  },
  {
    item: "Marketing Automation Tools",
    baseAmount: 1500,
    growthRate: 0.05,
    notes: "Email, CRM, marketing ops platforms"
  },
  {
    item: "Design Partner Program",
    baseAmount: 3000,
    growthRate: 0.1,
    notes: "Incentives, support for early adopters"
  },
  {
    item: "Community Building & Events",
    baseAmount: 2000,
    growthRate: 0.12,
    notes: "Meetups, conferences, community management"
  },
  {
    item: "Website & Landing Pages",
    baseAmount: 1000,
    growthRate: 0.03,
    notes: "Hosting, CMS, optimization tools"
  },
  {
    item: "Sales Tools (CRM, etc)",
    baseAmount: 1500,
    growthRate: 0.05,
    notes: "Salesforce, outreach tools, data enrichment"
  },
  {
    item: "Customer Success Tools",
    baseAmount: 1000,
    growthRate: 0.08,
    notes: "CS platforms, onboarding, support tools"
  },
  {
    item: "Marketing Contractors/Agencies",
    baseAmount: 4000,
    growthRate: 0.1,
    notes: "Specialized marketing support"
  },
  {
    item: "PR & Communications",
    baseAmount: 2000,
    growthRate: 0.05,
    notes: "PR agency, media relations, press releases"
  }
];

const rdExpensesData = [
  {
    item: "AI/ML Model Training Compute",
    baseAmount: 4000,
    growthRate: 0.12,
    notes: "GPU compute for model development and testing"
  },
  {
    item: "Research Tools & Subscriptions",
    baseAmount: 1500,
    growthRate: 0.03,
    notes: "Academic journals, research databases, APIs"
  },
  {
    item: "User Research & Testing",
    baseAmount: 2000,
    growthRate: 0.08,
    notes: "User interviews, usability testing, research tools"
  },
  {
    item: "Product Analytics Tools",
    baseAmount: 1000,
    growthRate: 0.05,
    notes: "Analytics platforms, session replay, heatmaps"
  },
  {
    item: "API & Integration Costs",
    baseAmount: 1500,
    growthRate: 0.1,
    notes: "Third-party APIs, integration platform fees"
  },
  {
    item: "Prototyping & Design Tools",
    baseAmount: 1000,
    growthRate: 0.03,
    notes: "Design software, prototyping tools, user testing"
  }
];
...
function addQuarterlyFormulas(sheet, row, firstMonthCol, quarterCols) {
  ...
  sheet.getCell(row, q1_2026).value = {
    formula: `SUM(${colToLetter(firstMonthCol)}${row}:${colToLetter(firstMonthCol + 2)}${row})`
  };
  sheet.getCell(row, q1_2026).numFmt = "$#,##0";

  sheet.getCell(row, q2_2026).value = {
    formula: `SUM(${colToLetter(firstMonthCol + 3)}${row}:${colToLetter(firstMonthCol + 5)}${row})`
  };
  sheet.getCell(row, q2_2026).numFmt = "$#,##0";

  sheet.getCell(row, q3_2026).value = {
    formula: `SUM(${colToLetter(firstMonthCol + 6)}${row}:${colToLetter(firstMonthCol + 8)}${row})`
  };
  sheet.getCell(row, q3_2026).numFmt = "$#,##0";

  sheet.getCell(row, q4_2026).value = {
    formula: `SUM(${colToLetter(firstMonthCol + 9)}${row}:${colToLetter(firstMonthCol + 11)}${row})`
  };
  sheet.getCell(row, q4_2026).numFmt = "$#,##0";

  sheet.getCell(row, total_2026).value = {
    formula: `SUM(${colToLetter(q1_2026)}${row}:${colToLetter(q4_2026)}${row})`
  };
  sheet.getCell(row, total_2026).numFmt = "$#,##0";
  sheet.getCell(row, total_2026).font = { bold: true };
  sheet.getCell(row, total_2026).fill = {
    type: "pattern",
    pattern: "solid",
    fgColor: { argb: "FFFFE699" }
  };

  sheet.getCell(row, q1_2027).value = {
    formula: `SUM(${colToLetter(firstMonthCol + 12)}${row}:${colToLetter(firstMonthCol + 14)}${row})`
  };
  sheet.getCell(row, q1_2027).numFmt = "$#,##0";

  sheet.getCell(row, q2_2027).value = {
    formula: `SUM(${colToLetter(firstMonthCol + 15)}${row}:${colToLetter(firstMonthCol + 17)}${row})`
  };
  sheet.getCell(row, q2_2027).numFmt = "$#,##0";

  sheet.getCell(row, q3_2027).value = {
    formula: `SUM(${colToLetter(firstMonthCol + 18)}${row}:${colToLetter(firstMonthCol + 20)}${row})`
  };
  sheet.getCell(row, q3_2027).numFmt = "$#,##0";

  sheet.getCell(row, q4_2027).value = {
    formula: `SUM(${colToLetter(firstMonthCol + 21)}${row}:${colToLetter(firstMonthCol + 23)}${row})`
  };
  sheet.getCell(row, q4_2027).numFmt = "$#,##0";

  sheet.getCell(row, total_2027).value = {
    formula: `SUM(${colToLetter(q1_2027)}${row}:${colToLetter(q4_2027)}${row})`
  };
  sheet.getCell(row, total_2027).numFmt = "$#,##0";
  sheet.getCell(row, total_2027).font = { bold: true };
  sheet.getCell(row, total_2027).fill = {
    type: "pattern",
    pattern: "solid",
    fgColor: { argb: "FFFFE699" }
  };

  sheet.getCell(row, grandTotal).value = {
    formula: `${colToLetter(total_2026)}${row}+${colToLetter(total_2027)}${row}`
  };
  sheet.getCell(row, grandTotal).numFmt = "$#,##0";
  sheet.getCell(row, grandTotal).font = { bold: true };
  sheet.getCell(row, grandTotal).fill = {
    type: "pattern",
    pattern: "solid",
...
function addCumulativeRow(
  ...
  for (let m = 0; m < MONTHS; m++) {
    ...
    if (m === 0) {
      ...
      sheet.getCell(row, col).value = {
        formula: `${colToLetter(col)}${totalRow}`
      };
    } else {
      // Subsequent months = previous cumulative + this month's total
      sheet.getCell(row, col).value = {
        formula: `${colToLetter(col - 1)}${row}+${colToLetter(col)}${totalRow}`
      };
    }
  ...
  sheet.getCell(row, q1_2026).value = {
    formula: `${colToLetter(firstMonthCol + 2)}${row}`
  };
  sheet.getCell(row, q1_2026).numFmt = "$#,##0";
  sheet.getCell(row, q1_2026).font = { italic: true };
  sheet.getCell(row, q1_2026).fill = {
    type: "pattern",
    pattern: "solid",
    fgColor: { argb: "FFFFEFD5" }
  };
  ...
  sheet.getCell(row, q2_2026).value = {
    formula: `${colToLetter(firstMonthCol + 5)}${row}`
  };
  sheet.getCell(row, q2_2026).numFmt = "$#,##0";
  sheet.getCell(row, q2_2026).font = { italic: true };
  sheet.getCell(row, q2_2026).fill = {
    type: "pattern",
    pattern: "solid",
    fgColor: { argb: "FFFFEFD5" }
  };
  ...
  sheet.getCell(row, q3_2026).value = {
    formula: `${colToLetter(firstMonthCol + 8)}${row}`
  };
  sheet.getCell(row, q3_2026).numFmt = "$#,##0";
  sheet.getCell(row, q3_2026).font = { italic: true };
  sheet.getCell(row, q3_2026).fill = {
    type: "pattern",
    pattern: "solid",
    fgColor: { argb: "FFFFEFD5" }
  };
  ...
  sheet.getCell(row, q4_2026).value = {
    formula: `${colToLetter(firstMonthCol + 11)}${row}`
  };
  sheet.getCell(row, q4_2026).numFmt = "$#,##0";
  sheet.getCell(row, q4_2026).font = { italic: true };
  sheet.getCell(row, q4_2026).fill = {
    type: "pattern",
    pattern: "solid",
    fgColor: { argb: "FFFFEFD5" }
  };
  ...
  sheet.getCell(row, total_2026).value = {
    formula: `${colToLetter(q4_2026)}${row}`
  };
  sheet.getCell(row, total_2026).numFmt = "$#,##0";
  ...
  sheet.getCell(row, q1_2027).value = {
    formula: `${colToLetter(firstMonthCol + 14)}${row}`
  };
  sheet.getCell(row, q1_2027).numFmt = "$#,##0";
  sheet.getCell(row, q1_2027).font = { italic: true };
  sheet.getCell(row, q1_2027).fill = {
    type: "pattern",
    pattern: "solid",
    fgColor: { argb: "FFFFEFD5" }
  };
  ...
  sheet.getCell(row, q2_2027).value = {
    formula: `${colToLetter(firstMonthCol + 17)}${row}`
  };
  sheet.getCell(row, q2_2027).numFmt = "$#,##0";
  sheet.getCell(row, q2_2027).font = { italic: true };
  sheet.getCell(row, q2_2027).fill = {
    type: "pattern",
    pattern: "solid",
    fgColor: { argb: "FFFFEFD5" }
  };
  ...
  sheet.getCell(row, q3_2027).value = {
    formula: `${colToLetter(firstMonthCol + 20)}${row}`
  };
  sheet.getCell(row, q3_2027).numFmt = "$#,##0";
  sheet.getCell(row, q3_2027).font = { italic: true };
  sheet.getCell(row, q3_2027).fill = {
    type: "pattern",
    pattern: "solid",
    fgColor: { argb: "FFFFEFD5" }
  };
  ...
  sheet.getCell(row, q4_2027).value = {
    formula: `${colToLetter(firstMonthCol + 23)}${row}`
  };
  sheet.getCell(row, q4_2027).numFmt = "$#,##0";
  sheet.getCell(row, q4_2027).font = { italic: true };
  sheet.getCell(row, q4_2027).fill = {
    type: "pattern",
    pattern: "solid",
    fgColor: { argb: "FFFFEFD5" }
  };
  ...
  sheet.getCell(row, total_2027).value = {
    formula: `${colToLetter(q4_2027)}${row}`
  };
  sheet.getCell(row, total_2027).numFmt = "$#,##0";
...
async function buildHeadcountSheet(sheet, firstMonthCol, quarterCols) {
  ...
  sheet.getCell(row, 4).value = {
    formula: `SUM(${colToLetter(4)}${firstDataRow}:${colToLetter(4)}${lastPersonRow})`
  };
  sheet.getCell(row, 4).numFmt = "$#,##0";
  ...
  for (let col = monthStartCol; col <= quarterCols.grandTotal; col++) {
    sheet.getCell(row, col).value = {
      formula: `SUM(${colToLetter(col)}${firstDataRow}:${colToLetter(col)}${lastPersonRow})`
    };
    sheet.getCell(row, col).numFmt = "$#,##0";
  ...
  sheet.getCell(row, 4).value = {
    formula: `${colToLetter(4)}${baseSalaryRow}*${colToLetter(2)}${row}`
  };
  sheet.getCell(row, 4).numFmt = "$#,##0";

  for (let col = monthStartCol; col <= quarterCols.grandTotal; col++) {
    sheet.getCell(row, col).value = {
      formula: `${colToLetter(col)}${baseSalaryRow}*0.35`
    };
    sheet.getCell(row, col).numFmt = "$#,##0";
  }
...
async function buildExpenseSheet(
  sheet,
  title,
  expenseData,
  firstMonthCol,
  ...
  for (let col = monthStartCol; col <= quarterCols.grandTotal; col++) {
    sheet.getCell(row, col).value = {
      formula: `SUM(${colToLetter(col)}${firstDataRow}:${colToLetter(col)}${lastItemRow})`
    };
    sheet.getCell(row, col).numFmt = "$#,##0";
    sheet.getCell(row, col).font = { bold: true };
    sheet.getCell(row, col).fill = {
...
async function buildSummarySheet(
  ...
  summarySheet.getCell(row, 1).value =
    "Funding Round: Seed Extension | Target: $2.0M - $3.0M | Jan 2026 - Dec 2027";
  ...
  summarySheet.getCell(row, 1).fill = {
    type: "pattern",
    pattern: "solid",
  ...
  for (let m = 0; m < MONTHS; m++) {
    ...
    summarySheet.getCell(row, col).font = {
      bold: true,
      color: { argb: "FFFFFFFF" }
    };
  ...
  Object.values(quarterCols).forEach((col, idx) => {
    ...
    summarySheet.getCell(row, col).alignment = {
      horizontal: "center",
      wrapText: true
    };
  });
  ...
  for (let col = monthStartCol; col <= quarterCols.grandTotal; col++) {
    ...
    summarySheet.getCell(row, col).value = {
      formula: `'Headcount Detail'!${colToLetter(hcCol)}${sheetRows.headcount.totalRow}`
    };
  ...
  for (let col = monthStartCol; col <= quarterCols.grandTotal; col++) {
    ...
    summarySheet.getCell(row, col).value = {
      formula: `'Infrastructure Detail'!${colToLetter(infraCol)}${sheetRows.infrastructure.totalRow}`
    };
    summarySheet.getCell(row, col).numFmt = "$#,##0";
  }
  row++;
  ...
  for (let col = monthStartCol; col <= quarterCols.grandTotal; col++) {
    ...
    summarySheet.getCell(row, col).value = {
      formula: `'Go-to-Market Detail'!${colToLetter(gtmCol)}${sheetRows.gtm.totalRow}`
    };
    summarySheet.getCell(row, col).numFmt = "$#,##0";
  }
  row++;
  ...
  for (let col = monthStartCol; col <= quarterCols.grandTotal; col++) {
    ...
    summarySheet.getCell(row, col).value = {
      formula: `'R&D Detail'!${colToLetter(rdCol)}${sheetRows.rd.totalRow}`
    };
    summarySheet.getCell(row, col).numFmt = "$#,##0";
  }
  row++;
...
async function buildAllDetailSheet(
  allDetailSheet,
  firstMonthCol,
  quarterCols,
  sheetRows
  ...
  // Title
  allDetailSheet.getCell(row, 1).value =
    "EVOLVE - All Expense Details (24 Months)";
  allDetailSheet.getCell(row, 1).font = { size: 14, bold: true };
  row += 2;

  // Headers
  allDetailSheet.getCell(row, 1).value = "Category / Line Item";
  allDetailSheet.getCell(row, 1).font = {
    bold: true,
    color: { argb: "FFFFFFFF" }
  };
  allDetailSheet.getCell(row, 1).fill = {
    type: "pattern",
    pattern: "solid",
    fgColor: { argb: "FF203864" }
  };
  ...
  for (let m = 0; m < MONTHS; m++) {
    ...
    allDetailSheet.getCell(row, col).font = {
      bold: true,
      color: { argb: "FFFFFFFF" }
    };
    allDetailSheet.getCell(row, col).fill = {
      type: "pattern",
      pattern: "solid",
      fgColor: { argb: "FF203864" }
    };
  ...
  Object.values(quarterCols).forEach((col, idx) => {
    ...
    allDetailSheet.getCell(row, col).alignment = {
      horizontal: "center",
      wrapText: true
    };
  });
  ...
  headcountData.forEach((person, idx) => {
    ...
    allDetailSheet.getCell(row, 1).value = {
      formula: `"  "&'Headcount Detail'!A${hcDataRow}`
    };
    ...
    for (let col = monthStartCol; col <= quarterCols.grandTotal; col++) {
      ...
      allDetailSheet.getCell(row, col).value = {
        formula: `'Headcount Detail'!${colToLetter(hcCol)}${hcDataRow}`
      };
  ...
  for (let col = monthStartCol; col <= quarterCols.grandTotal; col++) {
    const hcCol = col - monthStartCol + 5;
    allDetailSheet.getCell(row, col).value = {
      formula: `'Headcount Detail'!${colToLetter(hcCol)}${sheetRows.headcount.baseSalaryRow}`
    };
    allDetailSheet.getCell(row, col).numFmt = "$#,##0";
  ...
  for (let col = monthStartCol; col <= quarterCols.grandTotal; col++) {
    const hcCol = col - monthStartCol + 5;
    allDetailSheet.getCell(row, col).value = {
      formula: `'Headcount Detail'!${colToLetter(hcCol)}${sheetRows.headcount.benefitsRow}`
    };
    allDetailSheet.getCell(row, col).numFmt = "$#,##0";
  }
  row++;
  ...
  for (let col = monthStartCol; col <= quarterCols.grandTotal; col++) {
    const hcCol = col - monthStartCol + 5;
    allDetailSheet.getCell(row, col).value = {
      formula: `'Headcount Detail'!${colToLetter(hcCol)}${sheetRows.headcount.recruitingRow}`
    };
    allDetailSheet.getCell(row, col).numFmt = "$#,##0";
  }
  row++;
  ...
  for (let col = monthStartCol; col <= quarterCols.grandTotal; col++) {
    const hcCol = col - monthStartCol + 5;
    allDetailSheet.getCell(row, col).value = {
      formula: `'Headcount Detail'!${colToLetter(hcCol)}${sheetRows.headcount.cumulativeRow}`
    };
    allDetailSheet.getCell(row, col).numFmt = "$#,##0";
  ...
  infrastructureData.forEach((item, idx) => {
    ...
    allDetailSheet.getCell(row, 1).value = {
      formula: `"  "&'Infrastructure Detail'!A${infraDataRow}`
    };
    allDetailSheet.getRow(row).outlineLevel = 1;
    ...
    for (let col = monthStartCol; col <= quarterCols.grandTotal; col++) {
      const infraCol = col - monthStartCol + 5;
      allDetailSheet.getCell(row, col).value = {
        formula: `'Infrastructure Detail'!${colToLetter(infraCol)}${infraDataRow}`
      };
      allDetailSheet.getCell(row, col).numFmt = "$#,##0";
    }
    row++;
  ...
  for (let col = monthStartCol; col <= quarterCols.grandTotal; col++) {
    const infraCol = col - monthStartCol + 5;
    allDetailSheet.getCell(row, col).value = {
      formula: `'Infrastructure Detail'!${colToLetter(infraCol)}${sheetRows.infrastructure.cumulativeRow}`
    };
    allDetailSheet.getCell(row, col).numFmt = "$#,##0";
    allDetailSheet.getCell(row, col).font = { italic: true };
    allDetailSheet.getCell(row, col).fill = {
  ...
  goToMarketData.forEach((item, idx) => {
    ...
    allDetailSheet.getCell(row, 1).value = {
      formula: `"  "&'Go-to-Market Detail'!A${gtmDataRow}`
    };
    allDetailSheet.getRow(row).outlineLevel = 1;
    ...
    for (let col = monthStartCol; col <= quarterCols.grandTotal; col++) {
      const gtmCol = col - monthStartCol + 5;
      allDetailSheet.getCell(row, col).value = {
        formula: `'Go-to-Market Detail'!${colToLetter(gtmCol)}${gtmDataRow}`
      };
      allDetailSheet.getCell(row, col).numFmt = "$#,##0";
    }
    row++;
  ...
  for (let col = monthStartCol; col <= quarterCols.grandTotal; col++) {
    const gtmCol = col - monthStartCol + 5;
    allDetailSheet.getCell(row, col).value = {
      formula: `'Go-to-Market Detail'!${colToLetter(gtmCol)}${sheetRows.gtm.cumulativeRow}`
    };
    allDetailSheet.getCell(row, col).numFmt = "$#,##0";
    allDetailSheet.getCell(row, col).font = { italic: true };
    allDetailSheet.getCell(row, col).fill = {
  ...
  rdExpensesData.forEach((item, idx) => {
    ...
    allDetailSheet.getCell(row, 1).value = {
      formula: `"  "&'R&D Detail'!A${rdDataRow}`
    };
    allDetailSheet.getRow(row).outlineLevel = 1;
    ...
    for (let col = monthStartCol; col <= quarterCols.grandTotal; col++) {
      const rdCol = col - monthStartCol + 5;
      allDetailSheet.getCell(row, col).value = {
        formula: `'R&D Detail'!${colToLetter(rdCol)}${rdDataRow}`
      };
      allDetailSheet.getCell(row, col).numFmt = "$#,##0";
    }
    row++;
  ...
  for (let col = monthStartCol; col <= quarterCols.grandTotal; col++) {
    const rdCol = col - monthStartCol + 5;
    allDetailSheet.getCell(row, col).value = {
      formula: `'R&D Detail'!${colToLetter(rdCol)}${sheetRows.rd.cumulativeRow}`
    };
    allDetailSheet.getCell(row, col).numFmt = "$#,##0";
    allDetailSheet.getCell(row, col).font = { italic: true };
    allDetailSheet.getCell(row, col).fill = {
...
async function generateExpenseModel() {
  ...
  const headcountRows = await buildHeadcountSheet(
    headcountSheet,
    detailSheetMonthCol,
    detailQuarterCols
  );
  const infraRows = await buildExpenseSheet(
    infraSheet,
  ...
  const gtmRows = await buildExpenseSheet(
    gtmSheet,
    "Go-to-Market",
    goToMarketData,
    detailSheetMonthCol,
  ...
  const rdRows = await buildExpenseSheet(
    rdSheet,
  ...
  const summaryRows = await buildSummarySheet(
    summarySheet,
    firstMonthCol,
    quarterCols,
    sheetRows
  ...
  console.log(
    "✓ All Detail sheet bug fixed (formulas now sum category totals)"
  );
